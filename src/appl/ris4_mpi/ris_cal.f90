...
!   origin and radial expectation values. This is the MPI version.     *
!                                                                      *
...
!
      ...
      IF (myid .EQ. 0) THEN
          ...
          YES2 = GETYN ()
      ENDIF
      CALL MPI_Bcast (YES2, 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, ierr)
...
!
      ...
      IF (AVAIL_OB) THEN
        J = INDEX(NAME,' ')
        IF (myid .EQ. 0) OPEN(UNIT=50,FILE = NAME(1:J-1)//'.IOB',STATUS='UNKNOWN'       &
            ,FORM='UNFORMATTED')
...
!
      IF (AVAIL_TB) THEN
        J = INDEX(NAME,' ')
        IF (myid .EQ. 0) OPEN(UNIT=51,FILE = NAME(1:J-1)//'.ITB',STATUS='UNKNOWN'       &
          ,FORM='UNFORMATTED')
...
!
      CALL gdsummpi(SMSC1, NVEC)
      CALL gdsummpi(SMSC2, NVEC)
      CALL gdsummpi(DENS1, NVEC)
      CALL gdsummpi(DENS2, NVEC)
      CALL gdsummpi(DENS3, NVEC)
      CALL gdsummpi(DENS4, NVEC)
      CALL gdsummpi(DENS5, NVEC)
      CALL gdsummpi(DENS6, NVEC)
      CALL gdsummpi(DENS7, NVEC)
      IF (YES2) THEN
          CALL gdsummpi(DENS1VEC, NVEC*NRNUC)
      ENDIF
