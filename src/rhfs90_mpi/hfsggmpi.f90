...
!               [MPI90]: MPI communication routines.                   *
!                                                                      *
...
!   Allocate local working arrays for each process
      CALL ALLOC (HFC_LOCAL, 5, NVEC*NVEC, 'HFC_LOCAL', 'HFS')
      CALL ALLOC (GJC_LOCAL, NVEC*NVEC, 'GJC_LOCAL', 'HFS')
      CALL ALLOC (DGJC_LOCAL, NVEC*NVEC, 'DGJC_LOCAL', 'HFS')
...
!   Distribute IC loop over MPI processes

      DO IC = 1 + MYID, NCF, NPROCS
...
!   Store the matrix element in the correct location

               IF (IDIFF == 0 .AND. IR >= IC) THEN
                  IF (KT == 1) THEN
                     HFC_LOCAL(1,NVEC*(IC-1)+IR) = ELEMNT
                     GJC_LOCAL(NVEC*(IC-1)+IR) = ELEMNTGJ
                     DGJC_LOCAL(NVEC*(IC-1)+IR) = ELEMNTDGJ
                  ELSE
                     HFC_LOCAL(3,NVEC*(IC-1)+IR) = ELEMNT
                  ENDIF
               ELSE IF (IDIFF == 2) THEN
                  IF (KT == 1) THEN
                     HFC_LOCAL(2,NVEC*(IC-1)+IR) = ELEMNT
                  ELSE
                     HFC_LOCAL(4,NVEC*(IC-1)+IR) = ELEMNT
                  ENDIF
               ELSE IF (IDIFF == 4 .AND. KT == 2) THEN
                  HFC_LOCAL(5,NVEC*(IC-1)+IR) = ELEMNT
               ENDIF
...
!   Only process 0 outputs results
      IF (MYID .EQ. 0) THEN

!   Unit conversion factors
         AUMHZ = CVAC*AUCM/1.0D+06
...
!   Calculate the F-dependent matrix elements
...
!   Clean up
      CALL DALLOC (HFC_LOCAL, 'HFC_LOCAL', 'HFS')
      CALL DALLOC (GJC_LOCAL, 'GJC_LOCAL', 'HFS')
      CALL DALLOC (DGJC_LOCAL, 'DGJC_LOCAL', 'HFS')
      CALL DALLOC (HFC, 'HFC', 'HFS')
      CALL DALLOC (GJC, 'GJC', 'HFS')
      CALL DALLOC (DGJC, 'DGJC', 'HFS')
